...
export default function AddLabResultModal({ isOpen, onClose, onAdd }) {
  ...
  const addRow = () => {
    if (rows.length < 10) {
      setRows([...rows, { name: "", value: "", normal: "", unit: "", status: "normal" }]);
    }
  };
  ...
  const submitHandler = async () => {
    ...
    const filteredRows = rows.filter(row => 
      row.name.trim() || row.value.trim() || row.normal.trim()
    );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
      <div 
        className="bg-[#11294B] p-6 !rounded-2xl w-full max-w-2xl text-white relative shadow-2xl border-2 animate-scale-in"
      ...
      >
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="p-2 !rounded-full" style={{ backgroundColor: 'rgba(22, 156, 246, 0.2)' }}>
                <Plus className="w-6 h-6" style={{ color: '#169CF6' }} />
              </div>
              <h2 className="text-2xl font-bold text-white">Add New Lab Result</h2>
            </div>
            <button 
              onClick={handleClose}
        ...
        {/* Main Test Name */}
        <div className="mb-6">
          <label className="text-white font-medium mb-2 block">Main Test Name *</label>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-white/60 text-sm">{rows.length} / 10 rows</span>
          </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className="w-full p-2 rounded-lg text-sm border border-white/10 bg-white/5 focus:border-[#169CF6] outline-none text-white"
                        >
                        ...
                        <div className={`w-3 h-3 rounded-full flex-shrink-0 ${
                          row.status === 'normal' ? 'bg-green-500' :
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 px-6 py-3 !rounded-lg font-medium transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            style={{ 
